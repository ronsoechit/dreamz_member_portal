...
$runnerMutexName = "Local\DreamzReserveLaptopPaymentRunner"
...
if (-not (Test-Path -LiteralPath $installRoot)) {
  Write-Host "Reserve laptop payment runner is not installed."
  exit 0
}
...
while ([DateTime]::UtcNow -lt $deadline) {
  ...
  if ($quietMutexChecks -ge 5) {
    Write-Host "Reserve laptop payment runner stopped cooperatively."
    exit 0
  }
