...
if (Test-Path -LiteralPath $manifestPath -PathType Leaf) {
  try {
    ...
    $manifestValid = (
      ...
      [string]$manifest.release -eq "dreamz_frontdesk_payment_runner" -and
...
$powershellScripts = @(
  Join-Path $packageRoot "process_fep_now.ps1"
  Join-Path $PSScriptRoot "Install-DreamzFrontdeskPaymentRunner.ps1"
  Join-Path $PSScriptRoot "Restore-DreamzFrontdeskPaymentRunner.ps1"
  Join-Path $PSScriptRoot "Get-DreamzFrontdeskPaymentRunnerInventory.ps1"
  Join-Path $PSScriptRoot "Test-DreamzFrontdeskPaymentRunnerRelease.ps1"
)
$parseErrors = @()
  ...
  $actualFiles |
    Where-Object {
      ...
      (
        ...
        (Split-Path -Leaf $_) -ne "Test-DreamzFrontdeskPaymentRunnerRelease.ps1"
      )
    }
)
$result = [ordered]@{
  schema = 2
  release = "dreamz_frontdesk_payment_runner"
  version = "1.0.0"
