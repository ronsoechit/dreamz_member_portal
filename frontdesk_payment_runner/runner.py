...
class RuntimeConfig:
    ...
    @property
    def expected_data_root(self) -> str:
        return ntpath.normpath(ntpath.join(self.source_root, "Data"))
...
def normalize_configured_source_root(value: object) -> str:
    text = str(value or "").strip()
    if not text:
        raise RunnerError("source_root_missing")
    ...
    if not normalized or normalized in {".", "\\"}:
        raise RunnerError("source_root_invalid")
    source_drive, _ = ntpath.splitdrive(normalized)
    if source_drive.casefold() == "x:":
        raise RunnerError("source_root_reserved_for_transfer")
    reserved = ntpath.normcase(ntpath.normpath(RESERVED_TRANSFER_ROOT))
    normalized_case = ntpath.normcase(normalized)
    if normalized_case == reserved or normalized_case.startswith(reserved + "\\"):
        raise RunnerError("source_root_reserved_for_transfer")
    return normalized
...
def load_runtime_config(path: Path) -> RuntimeConfig:
    if not path.is_file():
        raise RunnerError("runtime_config_missing")
    try:
...
class StatusStore:
    def __init__(self, path: Path, source_root: str = ""):
        self.path = path
        self.source_root = source_root

    def set_source_root(self, source_root: str) -> None:
        self.source_root = normalize_configured_source_root(source_root)
...
def native_computer_name() -> str | None:
    if os.name != "nt":
        return None
...
def exact_gymassistant_window_count(expected_data_root: str) -> int:
...
def source_is_available(source_root: str) -> bool:
    root = Path(source_root)
    return root.is_dir() and (root / "Data").is_dir()
...
def workstation_readiness(config: RuntimeConfig) -> Readiness:
    computer_name = native_computer_name()
    ...
    if not desktop.ready:
        return desktop
    if not source_is_available(config.source_root):
        return Readiness(False, "source_data_unavailable")
    exact_windows = exact_gymassistant_window_count(config.expected_data_root)
    if exact_windows == 0:
    ...
    if current_idle < max(1, int(config.idle_seconds)):
...
def process_one_update(
    ...
    *,
    readiness_check: Callable[[RuntimeConfig], Readiness] = workstation_readiness,
    writer_call: Callable[[dict, int, int, str], dict] = apply_official_writer,
    ...
    if str(result.get("status") or "").strip().lower() == "deferred":
        ...
        if deferred_reason:
            ...
            return "deferred"
    if (
        str(result.get("status") or "").strip().lower() != "applied"
...
def process_available_command(
    ...
    *,
    ...
    initial_ready = readiness_check(config)
    ...
    after_claim_ready = readiness_check(config)
    ...
    *,
    ...
    try:
        ...
        if not token:
            ...
            logger.error("state=configuration_error reason=payment_token_missing")
            return 2
        ...
        logger.info(
            "state=started version=%s agent=%s source=configured_payment_only",
            PACKAGE_VERSION,
            AGENT_ID,
        )
        while True:
...
def run_health_check(config_path: Path) -> int:
    ...
    try:
        ...
        payload = {
            ...
            "expected_data_root": config.expected_data_root,
            "receipt_count": receipts.count(),
