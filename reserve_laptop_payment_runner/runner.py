...
AGENT_LABEL = "Reserve laptop 8KM7V7D"
EXPECTED_COMPUTER = "DESKTOP-8KM7V7D"
EXPECTED_WINDOWS_USER = "ron"
SOURCE_ROOT = Path(r"\\DREAMZ-FRNTDSK\Gym Assistant 2.6")
EXPECTED_DATA_ROOT = r"\\DREAMZ-FRNTDSK\Gym Assistant 2.6\Data"
PORTAL_URL = "https://dreamzmemberportal-production.up.railway.app"
MUTEX_NAME = r"Local\DreamzReserveLaptopPaymentRunner"
...
def desktop_availability() -> Readiness:
    ...
    if os.environ.get("COMPUTERNAME", "").casefold() != EXPECTED_COMPUTER.casefold():
        return Readiness(False, "target_computer_mismatch")
    if os.environ.get("USERNAME", "").casefold() != EXPECTED_WINDOWS_USER.casefold():
        return Readiness(False, "target_user_mismatch")
...
def workstation_readiness(idle_seconds: int) -> Readiness:
    ...
    if not source_is_available():
        return Readiness(False, "frontdesk_share_unavailable")
...
class PortalClient:
    ...
    def _request(self, path: str, *, method: str = "GET", payload: dict | None = None) -> dict:
        ...
        headers = {
            ...
            "User-Agent": f"Dreamz-ReserveLaptopPaymentRunner/{PACKAGE_VERSION}",
...
def configure_logger(path: Path) -> logging.Logger:
    path.parent.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("reserve_laptop_payment_runner")
...
def run_preflight(config_path: Path) -> int:
    """Inspect only fixed target identity, desktop and Gym Assistant readiness."""
    try:
        config = load_runtime_config(config_path)
        readiness = workstation_readiness(config.idle_seconds)
        print(
            json.dumps(
                {
                    "ok": readiness.ready,
                    ...
                    "windows_user": EXPECTED_WINDOWS_USER,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Agent reserve_8km7v7d, host DESKTOP-8KM7V7D, Windows user ron, "
            "and the Frontdesk Gym Assistant SMB source are fixed."
        )
    )
    parser.add_argument(
        "--config",
    ...
    mode.add_argument(
        ...
        action="store_true",
        help=(
            "Inspect only fixed target identity, active desktop and exact "
            "Gym Assistant window readiness. Does not contact Portal, claim, "
            "or write."
        ),
    )
    return parser
