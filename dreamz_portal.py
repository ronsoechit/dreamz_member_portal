...
app.config["FEP_PAYMENT_SYNC_TOKEN_RON_LAPTOP"] = os.getenv(
    "FEP_PAYMENT_SYNC_TOKEN_RON_LAPTOP"
)
...
app.config["PORTAL_EXISTING_MEMBER_REVERIFICATION_PILOT_MEMBER_IDS"] = tuple(
    member_id.strip()
...
app.config["PORTAL_EXISTING_MEMBER_REVERIFICATION_PILOT_EMAILS"] = tuple(
    email.strip().lower()
...
app.config["EQUIPMENT_UPLOAD_ROOT"] = os.getenv(
    "EQUIPMENT_UPLOAD_ROOT",
    os.path.join(app.instance_path, "equipment_uploads"),
)
...
class FepPaymentProcessCommand(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    source = db.Column(db.String, nullable=False, index=True)
    status = db.Column(db.String, default="pending", nullable=False, index=True)
    target_agent = db.Column(db.String, default="frontdesk_dreamz", nullable=False, index=True)
    requested_limit = db.Column(db.Integer, default=25, nullable=False)
...
class EquipmentItem(db.Model):
    ...
    category_key = db.Column(db.String, db.ForeignKey("equipment_category.key"), nullable=False, index=True)
    subcategory = db.Column(db.String, index=True)
    primary_muscle_groups = db.Column(db.Text)
    secondary_muscle_groups = db.Column(db.Text)
    ...
    exercise_aliases = db.Column(db.Text)
    image_url = db.Column(db.String)
    thumbnail_url = db.Column(db.String)
    gallery_images = db.Column(db.Text)
    video_url = db.Column(db.String)
    demo_url = db.Column(db.String)
    location_zone = db.Column(db.String)
    status = db.Column(db.String, default="active", nullable=False, index=True)
    visibility = db.Column(db.String, default="members", nullable=False, index=True)
    is_new = db.Column(db.Boolean, default=False, nullable=False, index=True)
    new_since = db.Column(db.Date)
    new_until = db.Column(db.Date)
    promo_label = db.Column(db.String)
    available_for_ai_coach = db.Column(db.Boolean, default=True, nullable=False, index=True)
    sort_order = db.Column(db.Integer, default=0, nullable=False, index=True)
    internal_notes = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.now, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.now, nullable=False)

    category = db.relationship("EquipmentCategory", primaryjoin="EquipmentItem.category_key == EquipmentCategory.key")
...
EQUIPMENT_STATUSES = ["active", "out_of_service", "hidden"]
EQUIPMENT_VISIBILITY_OPTIONS = ["public", "members", "staff_only"]
...
EQUIPMENT_CATEGORIES = [
    ("strength_machines", "Strength Machines", 10),
    ("plate_loaded", "Plate Loaded", 20),
    ("cardio", "Cardio", 30),
    ("free_weights", "Free Weights", 40),
    ("functional_training", "Functional Training", 50),
    ("group_classes", "Group Classes", 60),
    ("boxing_dojo", "Boxing / Dojo", 70),
    ("core", "Core", 80),
    ("benches_racks", "Benches & Racks", 90),
...
def backfill_runtime_schema_defaults():
    ...
    db.session.execute(text(
        f"UPDATE equipment_category SET is_active = {true_value} WHERE is_active IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_category SET sort_order = 0 WHERE sort_order IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_category SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_category SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_item SET status = 'active' WHERE status IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_item SET visibility = 'members' WHERE visibility IS NULL"
    ))
    db.session.execute(text(
        f"UPDATE equipment_item SET is_new = {sql_bool(False)} WHERE is_new IS NULL"
    ))
    db.session.execute(text(
        f"UPDATE equipment_item SET available_for_ai_coach = {true_value} WHERE available_for_ai_coach IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_item SET sort_order = 0 WHERE sort_order IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_item SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL"
    ))
    db.session.execute(text(
        "UPDATE equipment_item SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL"
    ))
    db.session.commit()
...
def json_list_text(values):
    return json.dumps(json_list(values))


def localized_json_text(value, language=None):
    ...
    try:
        ...
        if isinstance(parsed, dict):
            return parsed.get(language) or parsed.get(DEFAULT_LANGUAGE) or next(iter(parsed.values()), "")
    except (TypeError, ValueError):
        pass
    return str(value)


def equipment_category_label(category_key, language=None):
    key = f"equipment_category_{category_key}"
    label = translated_text(key, language or current_language())
    return label if label != key else (category_key or "").replace("_", " ").title()


def equipment_item_description(item, language=None):
...
def equipment_gallery(item):
    ...
    if item.image_url and item.image_url not in gallery:
        gallery.insert(0, item.image_url)
    return gallery
...
def member_visible_equipment_query():
    return EquipmentItem.query.filter(
        ...
        EquipmentItem.visibility.in_(["members", "public"]),
    )


def equipment_seed_defaults(item):
    slug = item.get("slug") or slugify(item.get("name"))
    return {
        "name": item.get("name", slug.replace("-", " ").title()),
        "slug": slug,
        "brand": item.get("brand") or "Dreamz",
        "line": item.get("line"),
        "category_key": item.get("category_key") or "strength_machines",
        "subcategory": item.get("subcategory"),
        "primary_muscle_groups": json_list_text(item.get("primary_muscle_groups")),
        "secondary_muscle_groups": json_list_text(item.get("secondary_muscle_groups")),
        "usage_description": json.dumps(item.get("usage_description") or {}),
        "setup_instructions": item.get("setup_instructions"),
        "beginner_tips": item.get("beginner_tips"),
        "common_mistakes": item.get("common_mistakes"),
        "alternatives": json_list_text(item.get("alternatives")),
        "exercise_aliases": json_list_text(item.get("exercise_aliases") or [item.get("name", "")]),
        "image_url": item.get("image_url"),
        "thumbnail_url": item.get("thumbnail_url"),
        "gallery_images": json_list_text(item.get("gallery_images")),
        "video_url": item.get("video_url"),
        "demo_url": item.get("demo_url"),
        "location_zone": item.get("location_zone"),
        "status": item.get("status") or "active",
        "visibility": item.get("visibility") or "members",
        "is_new": bool(item.get("is_new", False)),
        "new_since": parse_optional_date(item.get("new_since")),
        "new_until": parse_optional_date(item.get("new_until")),
        "promo_label": item.get("promo_label"),
        "available_for_ai_coach": bool(item.get("available_for_ai_coach", True)),
...
def seed_equipment_library():
    ...
    for key, name, sort_order in EQUIPMENT_CATEGORIES:
        ...
        if not category:
            category = EquipmentCategory(key=key, name=name, sort_order=sort_order)
            db.session.add(category)
        else:
            category.name = category.name or name
            category.sort_order = category.sort_order if category.sort_order is not None else sort_order

    seed_items = []
    if EQUIPMENT_SEED_PATH.exists():
        try:
            seed_items = json.loads(EQUIPMENT_SEED_PATH.read_text(encoding="utf-8"))
        except (OSError, ValueError):
            app.logger.exception("Equipment seed file could not be read.")
            seed_items = []
    if isinstance(seed_items, dict):
        seed_items = seed_items.get("items", [])
    ...
    for raw_item in seed_items:
        ...
        if not item:
            ...
            db.session.add(item)
            continue
        # Keep admin-edited descriptive fields intact; only backfill missing operational defaults.
        for field, value in defaults.items():
...
def equipment_admin_context():
    ...
    brands = [
        row[0] for row in db.session.query(EquipmentItem.brand)
        .filter(EquipmentItem.brand.isnot(None), EquipmentItem.brand != "")
        .distinct()
        .order_by(EquipmentItem.brand.asc())
        .all()
    ]
    return {
...
def empty_equipment_admin_context():
    return {
        "equipment_items": [],
        "equipment_categories": [],
        "equipment_statuses": EQUIPMENT_STATUSES,
...
def equipment_context_for_ai(member=None, focus_text=None, limit=18):
    def query_items():
        query = EquipmentItem.query.filter(
            EquipmentItem.available_for_ai_coach.is_(True),
            EquipmentItem.status == "active",
            EquipmentItem.visibility.in_(["members", "public"]),
        )
        focus = (focus_text or "").lower()
        if focus:
            terms = [term for term in re.split(r"[^a-z0-9]+", focus) if len(term) > 2]
            filters = []
            for term in terms[:5]:
                like = f"%{term}%"
                filters.extend([
        ...
        return query.order_by(EquipmentItem.is_new.desc(), EquipmentItem.sort_order.asc(), EquipmentItem.name.asc()).limit(limit).all()

    items = portal_context_value("equipment_context", [], query_items)
    if not items:
        return "equipment_library=unavailable"
    parts = []
    for item in items:
        muscles = ", ".join(json_list(item.primary_muscle_groups)[:3]) or "unknown"
        aliases = ", ".join(json_list(item.exercise_aliases)[:4]) or item.name
        parts.append(
            f"{item.name} category={item.category_key} muscles={muscles} aliases={aliases} status={item.status}"
        )
    return "equipment_library=" + "; ".join(parts)


def matching_equipment_for_exercise(exercise_name, equipment_name=None):
    haystack = f"{exercise_name or ''} {equipment_name or ''}".lower()
    if not haystack.strip():
        return None
    items = portal_context_value(
        "matching_equipment",
        [],
        lambda: EquipmentItem.query.filter(
            EquipmentItem.status == "active",
            EquipmentItem.visibility.in_(["members", "public"]),
        ).order_by(EquipmentItem.is_new.desc(), EquipmentItem.sort_order.asc()).all(),
    )
    for item in items:
        if any(alias and alias in haystack for alias in equipment_item_aliases(item)):
            return item
    return None
...
def empty_equipment_member_context():
    return {
        "equipment_items": [],
        "equipment_categories": [],
        "equipment_muscle_groups": EQUIPMENT_MUSCLE_GROUPS,
...
FEP_PAYMENT_COMMAND_STATUS_PENDING = "pending"
FEP_PAYMENT_COMMAND_STATUS_RUNNING = "running"
FEP_PAYMENT_COMMAND_STATUS_COMPLETED = "completed"
FEP_PAYMENT_COMMAND_STATUS_FAILED = "failed"
...
def fep_payment_command_claim_seconds():
    try:
        return max(60, min(int(os.getenv("FEP_PAYMENT_COMMAND_CLAIM_SECONDS", "900")), 3600))
    except ValueError:
        return 900


def fep_payment_command_agent_lock_key(agent_id):
    return int.from_bytes(
        hashlib.sha256(
            f"dreamz-fep-payment-command:{str(agent_id or '').strip()}".encode("utf-8")
        ).digest()[:8],
        byteorder="big",
        signed=True,
    )
...
def fep_payment_writer_lock_key():
    return int.from_bytes(
        hashlib.sha256(FEP_PAYMENT_WRITER_LOCK_NAMESPACE.encode("utf-8")).digest()[:8],
        byteorder="big",
        signed=True,
    )


def acquire_fep_payment_writer_lock():
    ...
    db.session.execute(
        ...
        {"lock_key": fep_payment_writer_lock_key()},
    )


def acquire_fep_payment_command_agent_lock(agent_id):
    if db.session.get_bind().dialect.name != "postgresql":
        return
    from sqlalchemy import text
    ...
    db.session.execute(
        text("SELECT pg_advisory_xact_lock(:lock_key)"),
        {"lock_key": fep_payment_command_agent_lock_key(agent_id)},
    )


@contextmanager
def hold_fep_payment_writer_lock(agent_id=None):
    """Serialize payment writers globally; optionally lock one agent second.

    PostgreSQL transaction advisory locks cover all web workers until the
    request commits or rolls back. SQLite is used only for local/test runs, so
    an in-process re-entrant lock gives those runs the same ordering semantics.
    """
    ...
    with _FEP_PAYMENT_WRITER_THREAD_LOCK:
        yield
...
def normalize_fep_payment_agent(value, *, default=FEP_PAYMENT_AGENT_FRONTDESK, strict=False):
    ...
    aliases = {
        ...
        "office": FEP_PAYMENT_AGENT_DREAMZ_OFFICE,
        "office_pc": FEP_PAYMENT_AGENT_DREAMZ_OFFICE,
...
def fep_payment_command_public(command, duplicate=False, pending_payments=None):
    ...
    if command.started_at:
        payload["started_at"] = command.started_at.isoformat()
    if command.completed_at:
        payload["completed_at"] = command.completed_at.isoformat()
    if command.claimed_by:
        payload["claimed_by"] = command.claimed_by
    if command.claim_expires_at:
        payload["claim_expires_at"] = command.claim_expires_at.isoformat()
    if command.error:
...
def create_fep_payment_process_command(payload):
    if not isinstance(payload, dict):
        raise FepPaymentReject("Expected JSON payment process request payload.", 400)

    source = str(payload.get("source") or "fep_manager_payment_process_request").strip()
    target_agent = normalize_fep_payment_agent(payload.get("target_agent"), strict=True)
    requested_limit = parse_fep_int(payload.get("limit"), "limit", required=False) or 25
    requested_limit = max(1, min(int(requested_limit), FEP_PAYMENT_PROCESS_REQUEST_MAX_LIMIT))
    requested_by = str(payload.get("requested_by") or payload.get("actor") or "").strip() or "fep_manager"
    upload_id = parse_fep_int(payload.get("upload_id"), "upload_id", required=False)
    upload_filename = str(payload.get("upload_filename") or "").strip() or None

    now = datetime.now()
    with hold_fep_payment_writer_lock(target_agent):
        ...
        active_command = (
            ...
            .order_by(FepPaymentProcessCommand.created_at.asc(), FepPaymentProcessCommand.id.asc())
            .first()
        )
        if active_command:
            active_upload_id = getattr(active_command, "upload_id", None)
            if active_upload_id != upload_id:
                # Persist an expired-claim release and end both advisory-lock
                # scopes before returning the conflict.
                db.session.commit()
                raise FepPaymentReject(
        ...
        command = FepPaymentProcessCommand(
            ...
            request_payload_json=json_text(payload),
            created_at=now,
            updated_at=now,
        )
...
def portal_member_context_summary(member, profile, language=None):
    ...
    equipment_context = portal_context_value(
        "member_equipment_library",
        "equipment_library=unavailable",
        lambda: equipment_context_for_ai(member, limit=22),
    )
    ...
    return (
        ...
        "member_routes="
        "/dashboard, /coach, /nutrition, /progress, /group-classes, /equipment, /account, /account/membership, /account/billing, /account/agreements, /account/documents, /pricing"
    )
...
def is_equipment_question(message):
    text = (message or "").lower()
    terms = (
        "apparaat", "apparaten", "machine", "machines", "equipment", "toestel",
        "leg extension", "leg curl", "glute", "high row", "lat pulldown",
        "cardio", "dumbbell", "barbell", "kettlebell", "cable", "bench",
        "equipo", "maquina", "maquinas", "aparato", "aparatnan",
    )
    return any(term in text for term in terms)
...
def coach_portal_direct_reply(member, profile, user_message, language=None):
    ...
    if is_pricing_question(user_message):
        ...
        if items:
            ...
            return "\n".join(lines)
    if is_equipment_question(user_message):
        return coach_equipment_direct_reply(member, user_message, language)
    return None
...
def generate_coach_reply(member, profile, user_message=None, workout_logs=None, category="conversation"):
    ...
    prompt = (
        ...
        "The portal context can include Dreamz equipment_library with real available machines and aliases. Prefer Dreamz equipment when creating workout advice, demo guidance or alternatives, and avoid out_of_service equipment. If the member asks which machines are available, answer from the equipment_library and point them to /equipment. "
...
def configured_fep_payment_sync_tokens():
    return {
        agent_id: str(app.config.get(config_name) or "").strip()
        for agent_id, config_name in FEP_PAYMENT_DEDICATED_TOKEN_CONFIG.items()
        if str(app.config.get(config_name) or "").strip()
    }


def matching_fep_payment_sync_token_agents(supplied_token):
    if not supplied_token:
        return []
    return [
        agent_id
        for agent_id, expected_token in configured_fep_payment_sync_tokens().items()
        if secrets.compare_digest(str(supplied_token), expected_token)
    ]
...
def require_sync_access():
    ...
    if matching_fep_payment_sync_token_agents(supplied_token):
        abort(403, "Payment-only sync token cannot access this sync endpoint.")
...
def require_fep_payment_sync_access(agent_id):
    agent_id = normalize_fep_payment_agent(agent_id, strict=True)
    supplied_token = request.headers.get("X-Sync-Token")
    dedicated_tokens = configured_fep_payment_sync_tokens()
    matching_agents = matching_fep_payment_sync_token_agents(supplied_token)
    if len(matching_agents) > 1:
        abort(503, "Payment sync token configuration is ambiguous.")
    if matching_agents and matching_agents[0] != agent_id:
    ...
    if dedicated_token:
        if not supplied_token or not secrets.compare_digest(
            ...
            abort(403, "Dedicated payment sync token required for this agent.")
        return
...
PORTAL_INVITATION_RECONCILABLE_STATUSES = {
    ...
    "waiting_for_smtp_configuration",
...
def existing_member_reverification_smtp_enabled():
    """Require a real SMTP attempt before the protected pilot can become sent."""
    return app.config.get("EMAIL_DELIVERY_MODE") == "smtp"


def existing_member_reverification_pilot_allowed(record):
    member_values = portal_invitation_config_values(
        "PORTAL_EXISTING_MEMBER_REVERIFICATION_PILOT_MEMBER_IDS"
    )
    email_values = portal_invitation_config_values(
        "PORTAL_EXISTING_MEMBER_REVERIFICATION_PILOT_EMAILS"
    )
    member_ids = {
        str(member_id).strip()
    ...
    return (
        len(member_values) == 1
        and len(email_values) == 1
        and len(member_ids) == 1
        and len(allowed_email_hashes) == 1
        and str(record.member_id or "").strip() in member_ids
...
def portal_invitation_public(record, duplicate=False):
    ...
    if request_type == PORTAL_INVITATION_REQUEST_TYPE_EXISTING_MEMBER_REVERIFICATION:
        response["member_number"] = str(record.member_id or "").strip()
        response["idempotency_key"] = record.request_payload_hash
    return response
...
def reconcile_portal_invitation(record):
    ...
    if request_type == PORTAL_INVITATION_REQUEST_TYPE_EXISTING_MEMBER_REVERIFICATION:
        ...
        if not existing_member_reverification_smtp_enabled():
            return wait_for_portal_invitation(record, "waiting_for_smtp_configuration")
    ...
    if (
        request_type == PORTAL_INVITATION_REQUEST_TYPE_EXISTING_MEMBER_REVERIFICATION
        and delivery_result != "sent"
    ):
        return mark_portal_invitation_for_review(record, "email_delivery_not_confirmed")
...
def member_dashboard_context(member, staff_admin_view=False):
    ...
    new_equipment_items = optional_dashboard_value(
        "new_equipment_items",
        [],
        lambda: (
...
def api_signup_portal_invitation():
    ...
    if record:
        ...
        if (
            ...
            conflict = portal_invitation_public(record, duplicate=True)
...
@app.post("/api/fep/payment-process-request")
def api_fep_payment_process_request():
    require_fep_access()
    ensure_runtime_schema()
    payload = request.get_json(silent=True) or {}
    try:
        command, duplicate, pending_payments = create_fep_payment_process_command(payload)
    except FepPaymentReject as exc:
        db.session.rollback()
        return fep_error_response(str(exc), exc.status_code)
    except Exception as exc:
        db.session.rollback()
        app.logger.exception("FEP payment-process-request failed.")
        return fep_error_response(str(exc), 500)
    return jsonify(fep_payment_command_public(command, duplicate=duplicate, pending_payments=pending_payments))


@app.get("/api/fep/payment-process-request")
def api_fep_payment_process_request_status():
    require_fep_access()
    ensure_runtime_schema()
    try:
        command_id = parse_fep_int(request.args.get("command_id"), "command_id", required=False)
        if command_id:
            # target_agent is immutable for a command. This scalar lookup only
            # selects the advisory-lock key; command state is read after lock.
            stored_target_agent = (
                db.session.query(FepPaymentProcessCommand.target_agent)
                .filter(FepPaymentProcessCommand.id == command_id)
                .scalar()
            )
            if stored_target_agent is None:
                return fep_error_response("payment process request was not found.", 404)
            target_agent = normalize_fep_payment_agent(stored_target_agent, strict=True)
        else:
            target_agent = normalize_fep_payment_agent(request.args.get("target_agent"), strict=False)
    except FepPaymentReject as exc:
        return fep_error_response(str(exc), exc.status_code)
    with hold_fep_payment_writer_lock(target_agent):
        release_expired_fep_payment_process_commands(datetime.now())
        query = FepPaymentProcessCommand.query
        if command_id:
            query = query.filter(FepPaymentProcessCommand.id == command_id)
        else:
            query = query.filter(FepPaymentProcessCommand.target_agent == target_agent)
        command = query.order_by(FepPaymentProcessCommand.created_at.desc(), FepPaymentProcessCommand.id.desc()).first()
        if not command:
            db.session.commit()
            return fep_error_response("payment process request was not found.", 404)
        response_payload = fep_payment_command_public(
            command,
            pending_payments=count_pending_fep_payment_updates_for_agent(
...
def release_expired_fep_payment_process_commands(now=None, agent_id=None):
    ...
    expired_commands = (
        ...
        .filter(
            ...
            FepPaymentProcessCommand.claim_expires_at.isnot(None),
            FepPaymentProcessCommand.claim_expires_at < now,
        )
    )
    if agent_id is not None:
        expired_commands = expired_commands.filter(
            FepPaymentProcessCommand.target_agent == agent_id
        )
    released = expired_commands.update(
        {
            FepPaymentProcessCommand.status: FEP_PAYMENT_COMMAND_STATUS_PENDING,
            FepPaymentProcessCommand.claimed_by: None,
            FepPaymentProcessCommand.claimed_at: None,
            FepPaymentProcessCommand.claim_expires_at: None,
            FepPaymentProcessCommand.error: (
                "Previous command claim expired before the agent reported a result."
            ),
            FepPaymentProcessCommand.updated_at: now,
        },
        synchronize_session=False,
    )
    if released:
...
def running_fep_payment_process_commands(*, for_update=False):
    query = (
        FepPaymentProcessCommand.query
        .filter(
            FepPaymentProcessCommand.status == FEP_PAYMENT_COMMAND_STATUS_RUNNING,
...
def processing_fep_payment_updates(*, for_update=False):
    query = (
        FepPaymentUpdate.query
        .filter(FepPaymentUpdate.status == FEP_PAYMENT_STATUS_PROCESSING)
        .order_by(FepPaymentUpdate.claimed_at.asc(), FepPaymentUpdate.id.asc())
    )
    if for_update and db.session.get_bind().dialect.name == "postgresql":
        query = query.with_for_update()
    return query
...
def fep_payment_process_command_sync_item(command):
    ...
    return {
        ...
        "claim_expires_at": command.claim_expires_at.isoformat() if command.claim_expires_at else None,
        "created_at": command.created_at.isoformat() if command.created_at else None,
        "updated_at": command.updated_at.isoformat() if command.updated_at else None,
    }
...
def api_sync_fep_payment_process_commands():
    ...
    require_fep_payment_sync_access(agent_id)
    ensure_runtime_schema()
    peek_only = str(request.args.get("peek") or "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    with hold_fep_payment_writer_lock(agent_id):
        released_claims = release_expired_fep_payment_claims(now)
        released = release_expired_fep_payment_process_commands(now)
        running_commands = running_fep_payment_process_commands(for_update=True)
        processing_records = processing_fep_payment_updates(for_update=True).all()
        processing_count = len(processing_records)
        running_command = running_commands[0] if len(running_commands) == 1 else None
        writer_conflict = len(running_commands) > 1
        writer_busy = bool(running_commands or processing_count)
        next_pending_command = None
        if not writer_busy and not writer_conflict:
            next_pending_query = (
                FepPaymentProcessCommand.query
                .filter(
                    FepPaymentProcessCommand.target_agent == agent_id,
            ...
            if db.session.get_bind().dialect.name == "postgresql":
                next_pending_query = next_pending_query.with_for_update()
            next_pending_command = next_pending_query.first()

        command = None
        if next_pending_command is not None:
            command = next_pending_command
        if command:
            command.status = FEP_PAYMENT_COMMAND_STATUS_RUNNING
            command.claimed_by = agent_id
            command.claimed_at = now
            command.claim_expires_at = claim_until
        ...
        elif next_pending_command is not None:
            writer_agent_id = normalize_fep_payment_agent(
                next_pending_command.target_agent
            )
        db.session.commit()
        return {
            "status": "success",
            "agent_id": agent_id,
            "agent_label": fep_payment_agent_label(agent_id),
            "released_expired_claims": released_claims,
            "released_expired_commands": released,
            "command_in_progress": running_command is not None,
            "agent_command_in_progress": bool(
                running_command is not None and writer_agent_id == agent_id
            ),
            "writer_busy": writer_busy,
            "writer_reserved": next_pending_command is not None,
            "writer_agent_id": writer_agent_id,
            "writer_agent_label": (
                fep_payment_agent_label(writer_agent_id)
                if writer_agent_id
            ...
            "processing_updates": processing_count,
            "commands": [fep_payment_process_command_sync_item(command)] if command else [],
        }


@app.post("/api/sync/fep-payment-process-commands/<int:command_id>/result")
def api_sync_fep_payment_process_command_result(command_id):
    payload = request.get_json(silent=True) or {}
    agent_id = sync_request_agent_id()
    require_fep_payment_sync_access(agent_id)
    ensure_runtime_schema()
    ...
    now = datetime.now()
    with hold_fep_payment_writer_lock(agent_id):
        release_expired_fep_payment_claims(now)
        release_expired_fep_payment_process_commands(now)
        command_query = FepPaymentProcessCommand.query.filter(
            FepPaymentProcessCommand.id == command_id
        )
        if db.session.get_bind().dialect.name == "postgresql":
            command_query = command_query.with_for_update()
        command = command_query.first()
        if not command:
            db.session.commit()
            return {
                "ok": False,
                "error": "payment process command was not found.",
            }, 404
        target_agent = normalize_fep_payment_agent(command.target_agent)
        if target_agent != agent_id:
            db.session.commit()
            return {
                "ok": False,
                "error": f"payment process command belongs to {fep_payment_agent_label(target_agent)}.",
            }, 409
        claimed_by = str(getattr(command, "claimed_by", None) or "").strip()
        ...
        if result_status == "completed":
            processing_count = processing_fep_payment_updates().count()
            if processing_count:
                error = (
                    f"{processing_count} payment update(s) are still processing; "
                    "manual reconciliation is required before retrying."
                )
                failed_result = dict(payload)
                failed_result["server_error"] = error
                failed_result["processing_updates"] = processing_count
                command.status = FEP_PAYMENT_COMMAND_STATUS_FAILED
                command.result_json = json_text(failed_result)
                command.error = error
        ...
        command.status = (
            ...
            if result_status == "completed"
            else FEP_PAYMENT_COMMAND_STATUS_FAILED
        )
        command.result_json = json_text(payload)
        command.error = None if result_status == "completed" else str(
            payload.get("error") or "Payment process command failed."
        )
        command.claimed_by = None
        command.claimed_at = None
        command.claim_expires_at = None
...
def api_sync_fep_payment_updates():
    ...
    with hold_fep_payment_writer_lock(agent_id):
        ...
        active_command = (
            ...
            if (
                running_command is not None
                and normalize_fep_payment_agent(running_command.target_agent) == agent_id
                and str(getattr(running_command, "claimed_by", None) or "").strip() == agent_id
            )
            else None
        )
        pending_command = (
            pending_fep_payment_process_command_for_agent(agent_id)
            if running_command is None and not writer_conflict
            else None
        )
        processing_records = processing_fep_payment_updates(
            for_update=not peek_only
        ...
        command_upload_id = active_command.upload_id if active_command else None
        if active_command is not None and not peek_only and processing_count == 0:
            active_command.claim_expires_at = now + timedelta(
                seconds=fep_payment_command_claim_seconds()
            )
            active_command.updated_at = now

        can_claim_command_update = (
            active_command is not None
            and not writer_conflict
            and processing_count == 0
        )
        can_claim_legacy_update = (
            running_command is None
            and not writer_conflict
            and pending_command is None
            and processing_count == 0
        )
        can_offer_records = can_claim_command_update or can_claim_legacy_update
        records = (
        ...
        if not peek_only:
            for record in records:
                ...
                record.updated_at = now
        claimed_count = len(records) if not peek_only else 0
        effective_processing_count = processing_count + claimed_count
        ...
        return {
            ...
            "command_waiting_for_claim": bool(
                pending_command is not None
                and normalize_fep_payment_agent(pending_command.target_agent) == agent_id
            ),
            "command_in_progress": running_command is not None,
            "agent_command_in_progress": active_command is not None,
            "writer_busy": bool(running_commands or effective_processing_count),
            "writer_reserved": pending_command is not None,
            "writer_agent_id": writer_agent_id,
            "writer_agent_label": (
                fep_payment_agent_label(writer_agent_id)
            ...
            "writer_conflict": writer_conflict,
            "processing_updates": effective_processing_count,
...
def api_sync_fep_payment_update_result(update_id):
    ...
    with hold_fep_payment_writer_lock(agent_id):
        ...
        record_agent = normalize_fep_payment_agent(
            record.target_agent,
            strict=False,
        )
        if record_agent != agent_id:
            ...
            return {
                "ok": False,
                "error": f"payment update is claimed by {fep_payment_agent_label(record_agent)}.",
            }, 409
        ...
        claimed_by = str(getattr(record, "claimed_by", None) or "").strip()
        if claimed_by != agent_id:
            claim_owner = (
                fep_payment_agent_label(claimed_by)
                if claimed_by
                else "no sync agent"
            )
            db.session.commit()
        ...
        if len(running_commands) > 1:
            ...
            return {
                "ok": False,
                "error": "multiple Gym Assistant payment writers are marked running; manual reconciliation is required.",
            }, 409
        active_command = running_commands[0] if running_commands else None
        if active_command is not None:
            command_agent = normalize_fep_payment_agent(
                active_command.target_agent
            )
            command_claimed_by = str(active_command.claimed_by or "").strip()
            if command_agent != agent_id or command_claimed_by != agent_id:
                db.session.commit()
                return {
                    "ok": False,
                    "error": "another Gym Assistant payment writer owns the global lease.",
                }, 409
        if (
            active_command is not None
        ...
        if result_status == "applied":
            ...
            reason = str(
                ...
                or "Gym Assistant write-back deferred."
            )
            record.error = f"{reason} Manual retry is required."
        record.claimed_by = None
        record.claimed_at = None
        record.claim_expires_at = None
        if active_command is not None:
...
def save_equipment_upload(file_storage):
    ...
    if ext not in {".jpg", ".jpeg", ".png", ".webp"}:
        abort(400, translated_text("equipment_invalid_image", current_language()))
    os.makedirs(app.config["EQUIPMENT_UPLOAD_ROOT"], exist_ok=True)
    stored_name = f"{datetime.now().strftime('%Y%m%d%H%M%S')}-{secrets.token_hex(4)}{ext}"
    file_storage.save(os.path.join(app.config["EQUIPMENT_UPLOAD_ROOT"], stored_name))
    return url_for("equipment_uploaded_image", filename=stored_name)


@app.get("/equipment-image/<path:filename>")
def equipment_uploaded_image(filename):
    if not session.get("member_id") and not current_staff_role():
        abort(404)
    return send_from_directory(app.config["EQUIPMENT_UPLOAD_ROOT"], filename)


@app.get("/staff/equipment")
def staff_equipment():
    staff_role = require_staff_access()
    staff_warning = None
    try:
        context = equipment_admin_context()
    except Exception:
        db.session.rollback()
        app.logger.exception("Equipment admin data unavailable while rendering staff equipment.")
        staff_warning = staff_data_warning("staff_equipment_nav")
        context = empty_equipment_admin_context()
    return render_template(
...
@app.post("/staff/equipment/save")
def staff_equipment_save():
    validate_csrf_token()
    require_staff_access()
    ensure_runtime_schema()
    seed_equipment_library()
    ...
    if not name or not EquipmentCategory.query.filter_by(key=category_key).first():
        abort(400, translated_text("equipment_required_fields", current_language()))
    item_id = parse_optional_int(request.form.get("item_id"))
    item = db.session.get(EquipmentItem, item_id) if item_id else None
    if not item:
        item = EquipmentItem()
        db.session.add(item)
    ...
    item.subcategory = request.form.get("subcategory", "").strip() or None
    item.primary_muscle_groups = json_list_text(request.form.getlist("primary_muscle_groups") or request.form.get("primary_muscle_groups"))
    item.secondary_muscle_groups = json_list_text(request.form.getlist("secondary_muscle_groups") or request.form.get("secondary_muscle_groups"))
    item.usage_description = json.dumps({current_language(): request.form.get("usage_description", "").strip()}) if request.form.get("usage_description", "").strip() else item.usage_description
    item.setup_instructions = request.form.get("setup_instructions", "").strip() or None
    item.beginner_tips = request.form.get("beginner_tips", "").strip() or None
    item.common_mistakes = request.form.get("common_mistakes", "").strip() or None
    item.alternatives = json_list_text(request.form.get("alternatives"))
    item.exercise_aliases = json_list_text(request.form.get("exercise_aliases"))
    upload_url = save_equipment_upload(request.files.get("image_file"))
    item.image_url = upload_url or request.form.get("image_url", "").strip() or item.image_url
    item.thumbnail_url = item.thumbnail_url or item.image_url
    item.gallery_images = json_list_text(request.form.get("gallery_images"))
    item.video_url = request.form.get("video_url", "").strip() or None
    item.demo_url = request.form.get("demo_url", "").strip() or None
    item.location_zone = request.form.get("location_zone", "").strip() or None
    status = request.form.get("status", "active").strip()
    item.status = status if status in EQUIPMENT_STATUSES else "active"
    visibility = request.form.get("visibility", "members").strip()
    item.visibility = visibility if visibility in EQUIPMENT_VISIBILITY_OPTIONS else "members"
    item.is_new = request.form.get("is_new") == "1"
    item.new_since = parse_optional_date(request.form.get("new_since"))
    item.new_until = parse_optional_date(request.form.get("new_until"))
    item.promo_label = request.form.get("promo_label", "").strip() or None
    item.available_for_ai_coach = request.form.get("available_for_ai_coach") == "1"
...
def member_equipment_library():
    ...
    try:
        ...
        staff_warning = translated_text("equipment_empty", current_language())
        context = empty_equipment_member_context()
    return render_template(
        "equipment.html",
        member=member,
        staff_page_warning=staff_warning,
...
@app.get("/equipment/<slug>")
def member_equipment_detail(slug):
    member, redirect_response = current_member_or_redirect()
    if redirect_response:
        return redirect_response
    ensure_runtime_schema()
    ...
    return render_template(
        ...
        member=member,
        item=item,
        primary_groups=json_list(item.primary_muscle_groups),
...
def member_coach():
    ...
    return render_template(
        ...
        matching_equipment_for_exercise=matching_equipment_for_exercise,
    )
