...
from reserve_laptop_payment_runner import runner
...
class FixedScopeTests(unittest.TestCase):
    def test_runner_scope_is_fixed(self):
        self.assertEqual(runner.AGENT_ID, "reserve_8km7v7d")
        self.assertEqual(runner.EXPECTED_COMPUTER, "DESKTOP-8KM7V7D")
        self.assertEqual(runner.EXPECTED_WINDOWS_USER, "ron")
        self.assertEqual(
            str(runner.SOURCE_ROOT).rstrip("\\"),
            r"\\DREAMZ-FRNTDSK\Gym Assistant 2.6",
        )
        self.assertEqual(
            runner.EXPECTED_DATA_ROOT,
            r"\\DREAMZ-FRNTDSK\Gym Assistant 2.6\Data",
        )
        self.assertEqual(
            runner.PORTAL_URL,
    ...
    def test_cli_has_no_agent_source_or_token_override(self):
        ...
        self.assertIn("--preflight", option_names)
    ...
    def test_portal_client_claims_only_fixed_agent_command_endpoint(self):
        ...
        self.assertIn(
            "/api/sync/fep-payment-process-commands?agent_id=reserve_8km7v7d",
            request.full_url,
        )
        self.assertNotIn("unit-test-token", request.full_url)
        self.assertEqual(request.get_header("X-sync-agent"), "reserve_8km7v7d")
    ...
    def test_portal_client_posts_payment_result_without_secret_in_body_or_url(self):
        ...
        self.assertIn(
            "/api/sync/fep-payment-updates/7/result?agent_id=reserve_8km7v7d",
            request.full_url,
        )
        self.assertNotIn("unit-test-token", request.full_url)
        self.assertNotIn(b"unit-test-token", request.data)
        self.assertEqual(json.loads(request.data)["agent_id"], "reserve_8km7v7d")
    ...
    def test_portal_client_peeks_one_update_without_claiming(self):
        ...
        self.assertIn("agent_id=reserve_8km7v7d", request.full_url)
...
class PreflightTests(unittest.TestCase):
    def test_exact_title_parser_accepts_only_frontdesk_share_data(self):
        self.assertEqual(
            runner.gymassistant_title_data_path(
                r"Gym Assistant 2.6 [Path=\\DREAMZ-FRNTDSK\Gym Assistant 2.6\Data]"
            ),
...
class SanitizedStateTests(unittest.TestCase):
    def test_status_contains_only_operational_summary(self):
        ...
        self.assertEqual(payload["agent_id"], "reserve_8km7v7d")
    ...
    def test_preflight_never_contacts_portal_or_claims(self):
        with tempfile.TemporaryDirectory() as temporary:
            config_path = Path(temporary) / "runtime.json"
            output = io.StringIO()
            with (
                ...
                patch.object(
                    ...
                    side_effect=AssertionError(
                        "preflight must not contact Portal"
                    ),
                ),
                patch.object(
                    runner,
                ...
                exit_code = runner.run_preflight(config_path)

        self.assertEqual(exit_code, 0)
        payload = json.loads(output.getvalue())
        self.assertTrue(payload["ok"])
        self.assertFalse(payload["portal_contacted"])
        self.assertFalse(payload["command_claimed"])
        self.assertFalse(payload["writer_called"])
